    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                ...
                GameObject g = Instantiate(cube, cubeContainer.transform);
                NodeArray[x, y] = new Node(Wall, worldPoint, x, y, g);//Create a new node in the array.
            }
        }
    }
    ...
    public void Update()
    {
        if (NodeArray != null)
        {
            foreach (Node n in NodeArray)
            {
                if (n.bIsWall)
                {
                    n.cube.GetComponent<MeshRenderer>().material.color = Color.green;
                }
                else
                {
                    n.cube.GetComponent<MeshRenderer>().material.color = Color.yellow;
                }
                ...
                if (FinalPath != null)
                {
                    ...
                    {
                        n.cube.GetComponent<MeshRenderer>().material.color = Color.red;
                    }

                }
                n.cube.transform.position = n.vPosition;
            }
        }
    }
